...
easterBtn.className = "icon-btn"; 
easterBtn.title = "Toggle background"; 
easterBtn.innerHTML = '<i class="fa-solid fa-image"></i>'; 
easterBtn.style.display = easterActive ? "block" : "none";
...
function toggleEaster() { 
  ...
  localStorage.setItem("nova_easter", easterActive); 
  applyEaster(); 
  easterBtn.style.display = "block"; 
}

applyEaster();
easterBtn.addEventListener("click", toggleEaster);

// Re-apply background layout dynamically if the window resizes
...
function burstBackpackStars() {
  document.fonts.load('900 24px "Font Awesome 6 Free"').then(() => {
    backpackParticles = [];
